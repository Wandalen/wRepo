...
command.properties =
{
  ...
  timeRange : 'Alternative for option `periodic`. Define range of dates for commits.\n    Calculates option `periodic` using option `relative` :\n\t- `now` - adds deltas to the current time;\n\t- `commit` - uses date of first source commit as the base time to add deltas.\n    Accepts next formats :\n\t- string `auto` - range from date of first source commit to current date;\n\t- left bound of time range is underscore - range from date of first source commit to calculated end time. Example: [_,12h];\n\t- right bound of time range is underscore - from calculated start time to current date. Example: [1h,_];\n\t- full range. Example: [-2h,2h]\n    NOTE:\n\t- do not use option simultaneously with option `periodic`;\n\t- upper bound of time range defines maximal date of last reflected commit, but the date of last reflected commit can be less than the maximal date.',
  deviation : 'Option works with options `periodic` and `timeRange`. Defines deviation of commit date. Commits will be written with random date in defined date range. Accepts time in the next formats:\n\t- number in miliseconds. Example: 60000;\n\t- format "hh:mm:ss". Example: 00:01:00;\n\t- combination of time units: `d` - days, `h` - hours, `m` - minutes, `s` - seconds. Example: 2d 1h 3m.',
  verbosity : 'Set verbosity. Default is 1.',
  dry : 'Dry run without changes in repository. Default is 0. If `dry` is 1, then command will print list of files to change.',
};
